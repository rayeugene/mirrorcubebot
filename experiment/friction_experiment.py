...
from friction_setup import frictions
...
def main():
    for i, friction in enumerate(frictions):
        for rotation in ['U', 'F', 'R', 'U\'', 'F\'', 'R\'']:
            try:
                scenario_file = "models/friction/f_" +str(i) + "scenario.dmd.yaml"
                ...
                filename = 'experiment/friction_results/' + 'f_' + str(i) + '_' + rotation + '.json'
                with open(filename, "w") as file:
                    json.dump(result, file,)
            except:
                print(friction, rotation)
                print("failed!")
